...
const { onBack, appParams, queryId } = useJump()
...
const getData = () => {
  ...
  listData.value = winMessageData.filter(v => v.chatId === queryId.value).map(v => {
...
const onSend = (sendContent: string, state: 0 | 1 = 0) => {
  ...
  appParams({ key: 'uploadMessage', value: wholeListData.value, state: 1 })
...
<template>
  <div v-if="!loading" safe-area-inset-bottom class="private-chat_box">
    <VanNavBar :border="false" safe-area-inset-top :clickable="false" class="top-chat-navbar" @click-left="onBack">
      <template #left>
